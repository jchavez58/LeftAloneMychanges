...
//-----------------------------------------------------------------------------
//added
extern void JCimage(int w, int h, GLuint x ); 
class Global {
    public:
        int xres, yres;
        char keys[65536];
...
void init_opengl(void)
{
    ...
    //create opengl texture elements(added)
    glGenTextures(1, &gl.JoshuaCTexture);
    int w = img[0].width;
    int h = img[0].height;
    //copied from rainforest.cpp
...
void physics()
{
    ...
    if (gl.keys[XK_space]) {
     ...
     /*if (gl.keys[XK_C]) {
            ...
            r2.left = 300;
            r2.center = 500;
            ggprint8b(&r2, 500, c, "Joshua Chavez");
     }*/
    if (g.mouseThrustOn) {
...
void render()
{
    ...
    //Added
       JCimage(900, 800, gl.JoshuaCTexture);
}
